...
def get_dump_stat(dump_file, spec, hostip, host):
    ...
    dump_ctrl = 'dump/' + host + '-ctrl.pcap'
    cmd = 'sudo tcpdump \"(port 5000) or (vlan and port 5000)\" -r ' + dump_ctrl + ' -n -w dump/tmp-arbiter.pcap > logs/dump-tmp.log 2>&1'
    print cmd
    os.system(cmd)
    
    cmd = 'capinfos -Tm dump/tmp-arbiter.pcap'
    #print cmd
    ...
    for flow in spec: 
        ...
        if m != None:
            ...
            if dst == hostip or src == hostip:
                ...
                cmd = 'sudo tcpdump vlan and dst ' + dst + ' and src ' + src + ' and dst port ' + port + ' -r ' + dump_file + ' -n -w dump/tmp.pcap > logs/dump-tmp.log 2>&1'
                #print cmd
                os.system(cmd)
                ...
                if pkt == 0:
                    ...
                    if src == hostip:
                        g_stat[flow] = [x+y for x,y in zip(temp, (byt,delta,0,0))]
                    elif dst == hostip:
                        g_stat[flow] = [x+y for x,y in zip(temp, (0,0,byt,delta))]
...
def stat_summary(spec):
    ...
    for key in spec:
        ...
        sendsize = g_stat[key][0]
        receivesize = g_stat[key][2]
        finish_time = g_stat[key][3]

	flowsize = receivesize

        if  sendsize < spec[key]:
            result = "Incomplete send"
            g_correctness_list.append('s')
            success = 0
        elif receivesize < spec[key]:
            result = "Incomplete receive"
            g_correctness_list.append('r')
            success = 0
        else:
            result = "OK"
            g_correctness_list.append('c')
            success = 1
...
def dump_trace_analysis():
    ...
    for hostid in range(1,num_host+1):
        ...
        get_dump_stat(tracefile, traffic, hostip, host)
    ...
    for hostid in range(1,num_host+1):
        ...
        if g_stat[hostip] < 1:
            num_non_arbiter += 1
            result = "no arbiter control"
        else:
            result = "arbiter OK"
            
        print host, result, g_stat[hostip] 
    ...
    print "=== Overall result ==="
    print "non-arbiter hosts: ", num_non_arbiter
    print "incomplete send: " + str(g_correctness_list.count('s'))
    print "incomplete receive: " + str(g_correctness_list.count('r'))
    print "correct flows: " + str(g_correctness_list.count('c'))
...
if __name__=='__main__':
    ...
    for hostid in range(1,num_host+1): 
        host = 'h' + str(hostid)
        cmd = '~/mininet/util/m ' + host + ' sudo tcpdump -i ' + host +'-eth0' + ' -n -s 64 -B 8192 -w dump/' + host + '.pcap > logs/dump-' + host + '.log 2>&1 &'
        print cmd
        os.system(cmd)
    time.sleep(2)
    ...
    for hostid in range(1,num_host+1): 
        host = 'h' + str(hostid)
        cmd = '~/mininet/util/m ' + host + ' sudo tcpdump -i ' + host +'-eth1' + ' -n -s 64 -B 8192 -w dump/' + host + '-ctrl.pcap > logs/dump-' + host + '-ctrl.log 2>&1 &'
        print cmd
        os.system(cmd)
    time.sleep(2)
